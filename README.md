# Tailwind Viewer Config

![Screenshot of UI](https://github.com/daverivera/tailwind-viewer-config/raw/master/ui-example.jpg)

Tailwind Viewer Config is a local UI tool for visualizing your Tailwind CSS configuration file. Keep it open during development to quickly reference custom Tailwind values/classes. Easily navigate between sections of the configuration and copy class names to your clipboard by clicking on them.

**[Demo using the default Tailwind config](https://daverivera.github.io/tailwind-viewer-config/)**

## Installation

### NPX

Run `npx tailwind-viewer-config` from within the directory that contains your Tailwind configuration file.

### Globally

`npm i tailwind-viewer-config -g`

### Locally

`npm i tailwind-viewer-config -D`

When installing locally, you can add an entry into the package.json scripts field to run and open the viewer:

```json
"scripts": {
  "tailwind-viewer-config": "tailwind-viewer-config -o"
}
```

## Usage

Run the `tailwind-viewer-config` command from within the directory that contains your Tailwind configuration file.

## Commands

### serve (default)

The `serve` command is the default command. Running `tailwind-viewer-config` is the same as `tailwind-viewer-config serve`.
...
`tailwind-viewer-config export ./output-dir`
