...
const flattenColorPalette = require(flattenColorPalettePath)

const resolveConfigPath = (configPath) =>
    path.resolve(process.cwd(), configPath)

const resolveConfig = (tailwindConfig) => {
    // Colors
    const baseColors = defaultTheme?.colors() || {}
    const extendedColors = tailwindConfig?.theme?.extend?.colors || {}
    const colors = merge({}, baseColors, extendedColors)

    // Screens
    const baseScreens = defaultTheme?.screens || {}
    const extendedScreens = tailwindConfig?.theme?.extend?.screens || {}
    const screens = merge(baseScreens, extendedScreens)
